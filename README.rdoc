...
*table-for* is a simple gem used to render tables based on the given collection.

=== Simplest examle

  <%= table_for_collection @users do -%>
...
=== Simple examle
  ...
  <%= table_for_collection @users, :html => {
    :class => "simple-table",
    :id => "users",
    :width => "100%",
    :tr => {
...
=== Example with colorized rows
  ...
  <%= table_for_collection @users, :html => { :tr => { :class => "row" } }, :stripes => %w{even odd} do %>
    <% column :name %>
    <% column :email %>
    <% column :address %>
  <% end %>
